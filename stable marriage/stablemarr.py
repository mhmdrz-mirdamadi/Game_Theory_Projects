#gets the preference matrix of boys and girls and find a boy-optimal and a girl-optimal stable marriage.
...
def GetStableMarriage(Bprefer,Gprefer,BNumber,GNumber):
    flag=True
    B=[{"key":x,"last_Req": -1, "isEngaged":False} for x in range(BNumber)]
...
def printMatching(SM):
    for x in range (len(SM)):
        print(f'{x+1}<--------->{SM[x]+1}')

def printGraph(SM,BNumber,GNumber,code):
...
Bprefer=GetMatrix(BNumber)
Gprefer=GetMatrix(GNumber)

print("Boy optimal:")
BO=GetStableMarriage(Bprefer,Gprefer,BNumber,GNumber)
printMatching(BO)
printGraph(BO,BNumber,GNumber,0)

print("Girl optimal:")
GO=GetStableMarriage(Gprefer,Bprefer,GNumber,BNumber)
printMatching(GO)
printGraph(GO,GNumber,BNumber,1)